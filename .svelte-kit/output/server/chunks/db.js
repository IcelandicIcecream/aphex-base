...
//#endregion
//#region src/lib/server/db/adapters/pglite.ts
var schema$1 = {
	...cms_schema_exports,
	...auth_schema_exports
};
...
async function pgliteAdapter(config) {
	...
	if (!config.building && config.autoMigrate !== false) try {
		await migrate$1(drizzle$1({ client: pglite }), { migrationsFolder: resolve("drizzle") });
	} catch (error) {
		const code = error.cause?.code;
		if (code === "42710" || code === "42P07") throw new Error("Boot migration failed: the PGlite data dir already has the schema but no migration journal (likely created with `pnpm db:push`). Either set APHEX_DB_AUTO_MIGRATE=false, or delete the data dir to start fresh.", { cause: error });
		throw error;
	}
	return {
		client: pglite,
...
var database;
if (driver === "sqlite") database = await sqliteAdapter({
...
else if (driver === "pglite") database = await pgliteAdapter({
...
else database = await postgresAdapter({
