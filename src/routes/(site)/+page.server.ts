...
export const load: PageServerLoad = async ({ locals }) => {
	...
	// Expand asset references to real URLs, so the row thumbnails have something to
	// render. Without this an image field is just `{ asset: { _ref } }` and every
	// image comes out blank, with no error to explain why.
